...
const WelcomeScreen = ({ username, handleSendMessageClick, userMessage, setUserMessage, loading }) => {
	...
	<main className="h-full w-full flex flex-col items-center pt-25">
		...
		<h2 className="text-xl font-semibold self-start ml-80">Hi <span className="text-sky-500 text-2xl">{username}👋</span></h2>
		...
		<form onSubmit={handleSendMessageClick} className="userInput mt-8 flex items-center justify-center w-180 gap-4">
			<textarea type="text" value={userMessage} onChange={(e) => setUserMessage(e.target.value)} placeholder="How can I help you today?" className="py-3 h-35 w-150 text-sm bg-neutral-950 border border-white/50 outline-none hover:bg-neutral-900 duration-300 ease-in-out px-4 resize-none rounded-2xl" />
